...
connection.connect((err) => {
  if (err) {
    ...
    res.status(500).json({ error: "Error connecting to the database", details: err });
    return;
  }
...
const useApiGet = (app, url, tableName) => {
  // Handle preflight requests
  app.options(url, cors());

  app.get(url, (req, res) => {
    const query = `SELECT * FROM alasparagente.${tableName}`;
...
app.post(
  ...
  (req, res) => {
    ...
    const query =
      "INSERT INTO alasparagente.med_movimientos (date_received, principio_activo, forma, concentracion, presentacion, unidad_medida, vigencia_invima, new_registro_invima , registro_invima, lote, vencimiento, observaciones) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
app.post(
  ...
  (req, res) => {
    ...
    const query =
      "INSERT INTO alasparagente.brigadas (date_brigada, location_b) VALUES (?, ?)";

    connection.query(query, [date_brigada, location_b], (error, results) => {
      if (error) {
        console.error("Error executing query: ", error);
        return res.status(500).json({ error: "Error cargando nueva Brigada." });
      }

      // Respond with the newly created Brigada data
      res
        .status(201)
        .json({ id_brigada: results.insertId, date_brigada, location_b });
    });
  }
);
...
app.post(
  ...
  (req, res) => {
    ...
    const { brigada, medicine, origen, destination, quantity } = req.body;

    const date_mov = new Date(); // Get the current date and time

    const query =
      "INSERT INTO alasparagente.med_movimientos (date_mov, brigada, medicine, origen, destination, quantity) VALUES (?, ?, ?, ?, ?, ?)";

    connection.query(
      query,
      [date_mov, brigada, medicine, origen, destination, quantity],
      (error, results) => {
        if (error) {
          console.error("Error executing query: ", error);
          return res
...
app.post(
  ...
  (req, res) => {
    ...
    const { id_num_doc, location_b, medicine, quantity } = req.body;

    const date_farma = new Date(); // Get the current date and time

    const query =
      "INSERT INTO alasparagente.farma (id_num_doc, location_b, medicine, quantity, date_farma) VALUES (?, ?, ?, ?, ?)";

    connection.query(
      query,
      [id_num_doc, location_b, medicine, quantity, date_farma],
      (error, results) => {
        if (error) {
          console.error("Error executing query: ", error);
