...
const useApiGet = (app, url, tableName, hasVoidedColumn = true) => {
  ...
  app.get(url, (req, res) => {
    ...
    connection.query(query, (error, results) => {
      ...
      res.header("Access-Control-Allow-Origin", "http://192.168.10.12:3000");
      res.header(
...
app.post(
  ...
  (req, res) => {
    ...
    const query =
      "INSERT INTO alasparagente.optometry (voided,id_num_doc,fecha_registro,mala_vision_lejos,mala_vision_cerca,ojo_rojo,prurito_ocular,dolor,hiperemia,lagrimeo,ardor_ocular,salto_renglon,cansancio_visual,caspa_parpados,otro,cual,diabetes,trauma_ocular,usa_correccion,hta,cirugia_ocular,cardiovasculares,vicios_reflaccion,artritis,ceguera,farmacos,otros,cuales,motivo_consulta,scvlod,scvloi,scvlao,scvpod,scvpoi,scvpao,ccvlod,ccvloi,ccvlao,ccvpod,ccvpoi,ccvpao,lensometria_od,lensometria_oi,tonometria_od,tonometria_oi,cover_test,cover_test_6m,cover_test_30cm,retinoscopia_od,retinoscopia_od_avvl,retinoscopia_od_avvp,retinoscopia_oi,retinoscopia_oi_avvl,retinoscopia_oi_avvp,subjetivo_od,subjetivo_od_av,subjetivo_od_add,subjetivo_oi,subjetivo_oi_av,subjetivo_oi_add,impresion_diagnostica,cie10,conducta,fflesfod,fflcilod,fflejeod,fflavod,fflesfoi,fflciloi,fflejeoi,fflavoi,ffcesfod,ffccilod,ffcejeod,ffcavod,ffcesfoi,ffcciloi,ffcejeoi,ffcavoi,addod,addoi,altura,tipolente,dnp,reffmontura,observalentes,medicamentos,rx,uso,control,) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
app.post(
  ...
  (req, res) => {
    ...
    const { voided, id_num_doc, location_b, diente, cara_diente, hallazgo } =
      req.body;
